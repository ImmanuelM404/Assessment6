...
const makeRobotDisplayCard = (bot) => {   //Displays card from above card created from html 
    return `
        <div class="bot-card outline">
        <img src='${bot.imgAddress}' alt='${bot.name}'/>
        <h3>${bot.name}</h3>
...
const renderPlayerDuo = () => {  // gives user bots selected as there duo 
...
const chooseBot = (id) => {
    if (playerDuo.length === 2) {
        return alert('You can only choose two bots!')   // stops user from picking more thsn two bots 
    }
...
const putBotBack = (id) => {   // adds bots after being romved by user 
...
const duel = () => {     // this is where potenial 'error dueling' is coming from, compDuo needs to be defined get result of Duel 
...
const getPlayerStats = () => {   // results from duel 
...
const getAllBots = () => {   // new game, user can select new bots to duel with 
